# 報表索引功能設計說明

## 📋 功能概述

報表索引功能提供一個統一的入口，讓使用者可以瀏覽並選擇特定模組下的所有報表。每個模組（如客戶、廠商）都有自己的報表中心。

## 🏗️ 架構設計

### 檔案結構

```
Models/
├── ReportDefinition.cs              # 報表定義模型
└── ReportCategoryConfig.cs          # 報表分類設定（標題、圖示）

Data/
└── ReportRegistry.cs                # 報表註冊表（集中管理）

Components/
├── Shared/
│   └── Report/
│       └── ReportIndexModalComponent.razor  # 通用報表索引 Modal
│
└── Pages/
    └── Reports/
        ├── GenericReportIndexPage.razor     # 通用報表中心（支援所有分類）
        └── AccountsReceivableReportPage.razor  # 應收帳款報表（現有）
```

### 組件關係圖

```
┌─────────────────────────────────────────────────────────────────┐
│                         MainLayout                               │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │      GenericReportIndexPage (Category=Customer/Supplier/...)  ││
│  │  ┌───────────────────────────────────────────────────────┐  ││
│  │  │           ReportIndexModalComponent                    │  ││
│  │  │  ┌─────────────────────────────────────────────────┐  │  ││
│  │  │  │        InteractiveTableComponent                 │  │  ││
│  │  │  │  顯示報表清單，每行有「列印」按鈕                │  │  ││
│  │  │  └─────────────────────────────────────────────────┘  │  ││
│  │  └───────────────────────────────────────────────────────┘  ││
│  └─────────────────────────────────────────────────────────────┘│
│                              ↓ 點擊列印                          │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │  AccountsReceivableReportPage（或其他報表 Modal）           ││
│  └─────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────┘
```

## 📖 使用方式

### 1. 新增報表定義

在 `Data/ReportRegistry.cs` 中加入報表定義：

```csharp
new ReportDefinition
{
    Id = "AR005",
    Name = "新報表名稱",
    Description = "報表說明",
    IconClass = "bi bi-file-earmark-text",
    Category = ReportCategory.Customer,  // 或其他分類
    RequiredPermission = "Customer.Read",
    ActionId = "OpenNewReport",          // Action 識別碼
    SortOrder = 5,
    IsEnabled = true                     // false 表示尚未實作
}
```

### 2. 建立報表 Modal 組件

參考 `AccountsReceivableReportPage.razor` 建立新的報表 Modal。

### 3. 在 MainLayout 中註冊 Action

```csharp
// 在 OnInitializedAsync 中加入
actionRegistry.Register("OpenNewReport", OpenNewReport);

// 加入開啟方法
public void OpenNewReport()
{
    showNewReport = true;
    StateHasChanged();
}
```

### 4. 導航選單整合

如需在導航選單中直接顯示報表入口，可在 `NavigationConfig.cs` 中使用：

```csharp
NavigationActionHelper.CreateActionItem(
    name: "報表名稱",
    description: "報表說明",
    iconClass: "bi bi-printer-fill",
    actionId: "OpenNewReport",
    category: "分類",
    requiredPermission: "Permission.Read",
    searchKeywords: new List<string> { "關鍵字" }
)
```

## 🔄 流程說明

1. 使用者從導航選單點擊「客戶報表中心」
2. 開啟 `GenericReportIndexPage` Modal（Category=Customer）
3. Modal 使用 `InteractiveTableComponent` 顯示所有客戶相關報表
4. 使用者點擊某報表的「列印」按鈕
5. 關閉報表索引 Modal
6. 透過 `OnReportSelected` 事件傳遞 ActionId 給 MainLayout
7. MainLayout 執行對應的 Action，開啟報表篩選 Modal

## 📊 報表分類常數

```csharp
public static class ReportCategory
{
    public const string Customer = "Customer";    // 客戶報表
    public const string Supplier = "Supplier";    // 廠商報表
    public const string Financial = "Financial";  // 財務報表
    public const string Inventory = "Inventory";  // 庫存報表
    public const string Sales = "Sales";          // 銷售報表
    public const string Purchase = "Purchase";    // 採購報表
}
```

## ✅ 目前已實作

| 報表中心 | Category 參數 | Action ID |
|----------|---------------|----------|
| 客戶報表中心 | `Customer` | `OpenCustomerReportIndex` |
| 廠商報表中心 | `Supplier` | `OpenSupplierReportIndex` |
| 庫存報表中心 | `Inventory` | `OpenInventoryReportIndex` |
| 銷售報表中心 | `Sales` | `OpenSalesReportIndex` |
| 採購報表中心 | `Purchase` | `OpenPurchaseReportIndex` |
| 財務報表中心 | `Financial` | `OpenFinancialReportIndex` |

## 📝 待實作報表

- [ ] 客戶對帳單 (`OpenCustomerStatementReport`)
- [ ] 客戶銷售分析 (`OpenCustomerSalesAnalysisReport`)
- [ ] 客戶交易明細 (`OpenCustomerTransactionReport`)
- [ ] 應付帳款報表 (`OpenAccountsPayableReport`)
- [ ] 廠商對帳單 (`OpenSupplierStatementReport`)
- [ ] 廠商進貨分析 (`OpenSupplierPurchaseAnalysisReport`)
