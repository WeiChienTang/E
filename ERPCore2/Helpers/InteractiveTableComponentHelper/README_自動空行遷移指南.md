...
> **2026-01-02 更新**: 遷移已全部完成，`AutoEmptyRowHelper.cs` 已刪除。
> 
> 所有 Table 元件現在統一使用 `InteractiveTableComponent` 的內建自動空行功能：
> - `EnableAutoEmptyRow` - 啟用自動空行
> - `DataLoadCompleted` - 控制載入時機
> - `CreateEmptyItem` - 建立空項目
> - `TriggerEmptyRowOnFilled` - 觸發欄位設定
