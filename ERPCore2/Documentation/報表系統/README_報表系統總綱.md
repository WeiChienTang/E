...
│  │   ・列印用：150 DPI（由 ReportPreviewModal 在列印前重新渲染）     │   │
│  │   ・PDF 用：150 DPI（由 ReportPreviewModal 在匯出 PDF 前渲染）   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                              ↓                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ 輸出路徑                                                          │   │
│  │ 列印：browserPrintImages(JS) → 瀏覽器 window.print() 本機列印   │   │
│  │ PDF ：PdfExportService → PuppeteerSharp → .pdf 下載              │   │
...
| 服務 | 說明 | 狀態 |
|------|------|------|
| FormattedPrintService | GDI+ 渲染引擎（Windows 專屬）。**僅負責渲染**：`RenderToImages()` 回傳 PNG byte[] 陣列，不直接列印。伺服器端印表機功能已移除。 | ✅ 完成 |
| ExcelExportService | ClosedXML 轉 .xlsx，跨平台 | ✅ 完成 |
| PdfExportService | PuppeteerSharp 將高解析度圖片合併為 PDF | ✅ 完成 |

> **列印流程（2026-03-13 起）**：所有列印均透過瀏覽器 `window.print()`，不再使用伺服器端印表機。`ReportPreviewModal` 在使用者按下「列印」時以 150 DPI 重新渲染後傳給 `browserPrintImages(JS)`，確保格式正確。
...
1. **Windows 專屬渲染**：`FormattedPrintService` 使用 GDI+（`System.Drawing`），僅能在 Windows 上執行。`[SupportedOSPlatform("windows6.1")]` 標記於服務上，非 Windows 環境（Linux/macOS 容器）無法使用。
2. **伺服器端列印已移除**：`PrinterConfiguration` 模組、`IPrinterConfigurationService`、`IReportPrintService` 等均已於 2026-03-13 刪除。所有列印一律透過瀏覽器 `window.print()`。
3. **DPI 說明**：
   - `PreviewImages`（96 DPI）：僅用於螢幕預覽，不可直接傳給列印
   - 列印 / PDF 均以 150 DPI 重新渲染，確保印出品質
4. **Excel 匯出跨平台**：`ExcelExportService` 使用 ClosedXML，支援所有平台
5. **單一報表 ID 原則**：每個單據類型只有一個報表 ID，入口點決定流程
6. **DynamicFilterTemplate 共用**：絕大多數報表共用單一 `DynamicFilterTemplate.razor`，篩選 UI 由 Criteria 屬性上的 `Filter*Attribute` 驅動，新增篩選欄位只需修改 Criteria 類別。唯一例外：PD003（商品條碼）使用 `ProductBarcodeBatchFilterTemplate`，因需要逐品項設定列印數量
