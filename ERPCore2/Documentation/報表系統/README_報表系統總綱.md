...
│                              ↓                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ FormattedPrintService  （GDI+ 渲染引擎，Windows 專屬）           │   │
│  │ → RenderToImages(doc, paper, dpi)   渲染為 PNG 圖片陣列          │   │
│  │   ・預覽用：96 DPI（由父組件呼叫，結果存入 PreviewImages）        │   │
│  │   ・PDF 用：150 DPI（由 ReportPreviewModal 在匯出 PDF 前渲染）   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                              ↓                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ 輸出路徑                                                          │   │
│  │ 列印：FormattedHtmlRenderer → @page{size:Xcm Ycm} HTML           │   │
│  │       → browserPrintHtml(JS) → iframe → window.print()          │   │
│  │ PDF ：FormattedPrintService（150 DPI）→ PdfExportService → .pdf  │   │
│  │ Excel：ExcelExportService → ClosedXML → .xlsx 下載               │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│   ├── IEntityReportService.cs             # 包含 BatchPreviewResult 定義（含 Documents）
│   └── I[Entity]ReportService.cs
├── FormattedPrintService.cs                # GDI+ 渲染，用於預覽圖片 / PDF
...
| 服務 | 說明 | 狀態 |
|------|------|------|
| FormattedPrintService | GDI+ 渲染引擎（Windows 專屬）。**僅負責渲染**：`RenderToImages()` 回傳 PNG byte[] 陣列，供預覽與 PDF 匯出使用。**列印不再使用此服務**，已改為 HTML 路徑。 | ✅ 完成 |
| FormattedHtmlRenderer | 純靜態類別（跨平台）。`RenderBatchToHtml(documents, paper)` 將 `FormattedDocument` 清單轉為含 `@page { size: Xcm Ycm }` 的完整 HTML 字串，供瀏覽器列印。 | ✅ 完成 |
| ExcelExportService | ClosedXML 轉 .xlsx，跨平台 | ✅ 完成 |
| PdfExportService | PuppeteerSharp 將高解析度圖片合併為 PDF | ✅ 完成 |

> **列印流程（2026-03-14 起，HTML 列印）**：按下「列印」時，`ReportPreviewModal` 呼叫 `FormattedHtmlRenderer.RenderBatchToHtml(documents, paper)` 產生含 `@page { size: Xcm Ycm }` 的 HTML 字串，再由 JS `browserPrintHtml()` 寫入隱藏 iframe 並呼叫 `window.print()`。瀏覽器直接按紙張 CSS 尺寸排版，完全不使用 GDI+ 渲染。`BatchPreviewResult.Documents` 保存每份獨立文件（保有正確表頭/頁尾）供批次列印使用。
...
1. **Windows 專屬渲染**：`FormattedPrintService` 使用 GDI+（`System.Drawing`），僅能在 Windows 上執行。`[SupportedOSPlatform("windows6.1")]` 標記於服務上，非 Windows 環境（Linux/macOS 容器）無法使用。**列印改用 `FormattedHtmlRenderer` 後，列印本身已跨平台；只有 PDF 匯出仍需 Windows。**
2. **伺服器端列印已移除**：`PrinterConfiguration` 模組、`IPrinterConfigurationService`、`IReportPrintService` 等均已於 2026-03-13 刪除（Migration `RemovePrinterConfiguration` 已執行）。所有列印一律透過 `FormattedHtmlRenderer → browserPrintHtml → window.print()`。
3. **`@page { size }` CSS 語法**：只允許 `size: Xcm Ycm`（長度）或 `size: A4 landscape`（預定義名稱 + 方向）。自定義長度後**不可**加 `landscape/portrait` 關鍵字，否則整個宣告失效，瀏覽器退回 A4。
4. **DPI / 列印格式說明**：
   - `PreviewImages`（96 DPI）：僅用於螢幕預覽，不可直接傳給列印或 PDF
   - **列印**：HTML 路徑（`FormattedHtmlRenderer`），無 DPI 概念，瀏覽器原生排版
