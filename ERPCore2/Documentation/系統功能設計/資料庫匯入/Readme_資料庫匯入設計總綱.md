...
當必填欄位既沒有對應來源、使用者也未填預設值時，系統提供「套用智慧預設」按鈕，依型別自動填入：
...
| **型別轉換** | Excel 數值列可能讀成 string、日期格式不一 | 每欄位自動嘗試型別轉換，失敗的在預覽中標記 |
...
Services/Import/
├── IDatabaseImportService.cs                 ← 介面（6 個方法）
└── DatabaseImportService.cs                  ← 實作（反射/ClosedXML/自動配對/型別轉換/EF Core Transaction）
