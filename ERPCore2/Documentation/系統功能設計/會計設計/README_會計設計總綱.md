...
| [README_會計_Phase3_帳務管理.md](README_會計_Phase3_帳務管理.md) | AR/AP 帳齡分析（FN012/FN013）、年底結帳（P3-B）；P3-C 已完成 | 🟡 中 |
...
| # | 問題 | 嚴重程度 | 修正計畫 |
|---|------|----------|----------|
| 1 | ~~試算表（FN006）缺少「期初餘額」欄，為五欄而非標準六欄~~ | ✅ 已修正 | Phase 1-C（2026-03-17） |
| 2 | ~~無法鎖定已關帳期間，任何時候都可補登歷史傳票~~ | ✅ 已修正 | Phase 1-A（2026-03-17） |
| 3 | 損益表（FN007）科目跨年不歸零，下年度試算表期初餘額包含歷史累計 | 🔴 高 | Phase 3-B（年底結帳） |
...
| 6 | 生產完工 `IsJournalized` 旗標尚未建立，設計上應置於 `ProductionScheduleCompletion`（分批完工記錄），而非 `ProductionScheduleItem`（排程主檔） | 🟠 中 | Phase 2-C |
