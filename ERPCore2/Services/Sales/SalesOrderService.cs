...
namespace ERPCore2.Services
{
    ...
    public class SalesOrderService : GenericManagementService<SalesOrder>, ISalesOrderService
    {
        ...
        protected override IQueryable<SalesOrder> BuildGetAllQuery(AppDbContext context)
        {
        ...
        public async Task<SalesOrder?> GetWithDetailsAsync(int orderId)
        {
            try
            {
                ...
                return await context.SalesOrders
                    ...
                    .Include(so => so.SalesOrderDetails)
                        .ThenInclude(sod => sod.QuotationDetail)
                            .ThenInclude(qd => qd!.Quotation)
                    .FirstOrDefaultAsync(so => so.Id == orderId);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ServiceResult> ApproveAsync(int id, int? approvedBy)
        {
            ...
            try
            {
                var entity = await context.SalesOrders.FirstOrDefaultAsync(x => x.Id == id);
                if (entity == null) return ServiceResult.Failure("找不到銷貨訂單");
                if (entity.IsApproved) return ServiceResult.Failure("銷貨訂單已核准，無需重複核准");
        ...
        public async Task<ServiceResult> RejectAsync(int id, int rejectedBy, string reason)
        {
            ...
            var entity = await context.SalesOrders.FirstOrDefaultAsync(x => x.Id == id);
            if (entity == null) return ServiceResult.Failure("找不到銷貨訂單");
            if (!string.IsNullOrEmpty(entity.RejectReason)) return ServiceResult.Failure("銷貨訂單已經駁回，無需重複駁回");
        ...
        public async Task<(List<SalesOrder> Items, int TotalCount)> GetPagedWithFiltersAsync(
            ...
            int pageSize)
        {
            ...
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                IQueryable<SalesOrder> query = context.SalesOrders.Include(so => so.Customer);
                if (filterFunc != null) query = filterFunc(query);
                var totalCount = await query.CountAsync();
                var items = await query
                    .OrderByDescending(so => so.OrderDate).ThenBy(so => so.Code)
