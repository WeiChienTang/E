    ...
    public class JournalEntryService : GenericManagementService<JournalEntry>, IJournalEntryService
    {
        ...
        public async Task<(bool Success, string ErrorMessage)> CancelDraftEntryAsync(int id, string updatedBy)
        {
            try
            {
                ...
                // 自動產生的傳票作廢後，重置來源業務單據的 IsJournalized，
                // 使其重新出現在待轉傳票清單（與 ReverseEntryAsync 行為一致）
                if (entry.EntryType == JournalEntryType.AutoGenerated &&
                    !string.IsNullOrWhiteSpace(entry.SourceDocumentType) && entry.SourceDocumentId.HasValue)
                {
                    await ResetSourceDocumentJournalizedAsync(context, entry.SourceDocumentType, entry.SourceDocumentId.Value);
                }
