...
namespace ERPCore2.Services
{
    ...
    public class SetoffPaymentDetailService : GenericManagementService<AccountsReceivableSetoffPaymentDetail>, ISetoffPaymentDetailService
    {
        ...
        public override async Task<List<AccountsReceivableSetoffPaymentDetail>> GetAllAsync()
        {
            try
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                return await context.AccountsReceivableSetoffPaymentDetails
                    .Include(d => d.Setoff)
            ...
            catch (Exception ex)
            {
                ...
                return new List<AccountsReceivableSetoffPaymentDetail>();
            }
        }

        public override async Task<AccountsReceivableSetoffPaymentDetail?> GetByIdAsync(int id)
        {
            try
            {
                using var context = await _contextFactory.CreateDbContextAsync();
        ...
        public override async Task<List<AccountsReceivableSetoffPaymentDetail>> SearchAsync(string searchTerm)
        {
            try
            {
        ...
        public override async Task<ServiceResult> ValidateAsync(AccountsReceivableSetoffPaymentDetail entity)
        {
            try
            {
                // 驗證付款方式
        ...
        public async Task<List<SetoffPaymentDetailDto>> GetBySetoffIdAsync(int setoffId)
        {
            ...
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                var details = await context.AccountsReceivableSetoffPaymentDetails
                    .Include(d => d.PaymentMethod)
                    .Include(d => d.Bank)
            ...
            List<int> deletedIds)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (deletedIds.Any())
                    {
                        var itemsToDelete = await context.AccountsReceivableSetoffPaymentDetails
                            .Where(d => deletedIds.Contains(d.Id))
                            .ToListAsync();

                        context.AccountsReceivableSetoffPaymentDetails.RemoveRange(itemsToDelete);
                    }
                    ...
                    foreach (var dto in validDetails)
                    {
                        ...
                        {
                            // 更新現有明細
                            var existing = await context.AccountsReceivableSetoffPaymentDetails
                            ...
                            if (existing != null)
                            {
                                ...
                                context.AccountsReceivableSetoffPaymentDetails.Update(existing);
                            }
                        }
                        else
                        {
                            // 新增明細
                            var newDetail = new AccountsReceivableSetoffPaymentDetail
                            {
                                SetoffId = setoffId,
                            ...
                            await context.AccountsReceivableSetoffPaymentDetails.AddAsync(newDetail);
                        }
                    }
        ...
        public async Task<bool> DeleteBySetoffIdAsync(int setoffId)
        {
            try
            {
                ...
                if (details.Any())
                {
                    context.AccountsReceivableSetoffPaymentDetails.RemoveRange(details);
                    await context.SaveChangesAsync();
                }
